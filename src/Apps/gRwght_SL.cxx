...
int main(int argc, char ** argv)
{
  ...
  //
  ofstream output;
  output.open("Reweighted.txt");
  output << "#Event      weight\n";
  TFile *outFile = new TFile("weights.root","recreate");
  TTree *weightTree = new TTree("weights","GENIE weight tree");
  double branch_weight;
  int branch_eventnum;
  weightTree->Branch("eventnum", &branch_eventnum,"eventnum/I");
  weightTree->Branch("weights", &branch_weight,"weights/D");
  ...
  for (int iev = nfirst; iev <= nlast; iev++) {
      ...
      output << setprecision(4) << iev << "         " << wght << " \n";
      branch_eventnum=idx;
      branch_weight=wght;
      weightTree->Fill();
      // Clean-up
